...
static long long getTimeInMs(void){
    ...
    // double milliSeconds_3decimal = (double)milliSeconds;
...
// Function to extract and process samples by the analysis module
    ...
    for (int i  = 0; i < bufferIndex; i += 2) {
        ...
        else if(readingCount > 1 ){ //Fron Second round, start recordinf the Time Interval min/max
            time_interval = (double)(time_current - time_previous);
            time_interval /= 1000;
        ...
        // Dip detection logic
        double voltageDiff = abs(a2dReading - a2d_exp_average) / 1000.0;
